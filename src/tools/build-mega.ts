...
[PATTERN] transition*/animation* longhands in cloudTokens.styles or inline styles: pass each as {type:"layers", value:[...]} or as {type:"unparsed", value:"<css>"}. Single typed values ({type:"var"}, {type:"keyword"}, {type:"unit"}) are auto-wrapped to layers[1]; missing longhands of the cohort are auto-completed with CSS defaults at matching layer count since v2.7.10. See pattern "transition-animation-format".`,
  ...
  create_navigation_menu: `Use when: one-call desktop mega-menu (Radix NavigationMenu) with flat links + optional mega panels + chevron rotation. Higher-level than push_fragment. Do NOT use when: needing a custom layout (use push_fragment). Returns: push result. Side effects: push. dryRun defaults true. Example: {action:"create_navigation_menu",label:"navmenu-desktop",projectSlug:"my-site",parentInstanceId:"headerId",items:[{kind:"link",label:"Home",href:"/"}]}`,
  create_popup: `Use when: one-call promo modal / newsletter popup (Radix Dialog + hidden auto-trigger + HtmlEmbed script with trigger mode & frequency + Radix-native close + sr-only a11y). Higher-level than push_fragment. Do NOT use when: needing a mobile drawer (use create_sheet) or a desktop mega menu (use create_navigation_menu) or a custom layout (use push_fragment). Returns: push result + IDs. Side effects: push. dryRun defaults true.
...
Example: {action:"create_popup",label:"popup-promo",projectSlug:"my-site",parentInstanceId:"pageRootId",content:{kind:"image",assetId:"913a...4db",alt:"Promo",href:"/offres"},trigger:{mode:"auto-delay",delayMs:3000},frequency:"once-per-session"}`,
  push_html: `Use when: convert raw HTML+CSS into a WebstudioFragment and push it (onboarding from an existing section). Limits: 1 root element only, no @keyframes, no <style> tags in HTML (pass CSS separately), media queries limited to Webstudio breakpoints (max-width: 991/767/479). Returns: push result + parse stats (rules applied/skipped). Side effects: push to Webstudio Cloud. Example: {action:"push_html",label:"import-section",projectSlug:"my-site",pushTo:{projectSlug:"my-site",parentInstanceId:"root",dryRun:true},html:"<div class='hero'><h1>Hello</h1></div>",css:".hero { padding: 40px; }"}`,
};
