...
const DESCRIPTIONS = {
  setup: `Use when: register Webstudio credentials (cookie + csrfToken + webstudioProjectId + appVersion) for a project in local config. Do NOT use when: project already authed and just toggling push permission (use action:"allow_push"). Returns: confirmation + local path. Side effects: writes to ~/.webstudio-mcp/projects/<slug>/webstudio-auth.json. Example: {action:"setup",label:"auth-my-site",projectSlug:"my-site",webstudioProjectId:"xxxx",cookie:"...",csrfToken:"...",allowPush:true}`,
  allow_push: `Use when: enable or disable push permission for a project (safety toggle, the session cookie grants access to ALL projects so allowPush prevents accidental pushes to wrong slug). Do NOT use when: setting up auth from scratch (use action:"setup"). Returns: new allowPush state. Side effects: writes to local auth file. Example: {action:"allow_push",label:"enable-push",projectSlug:"my-site",allow:true}`,
  update_app_version: `Use when: bump the Webstudio appVersion (x-webstudio-client-version) when Webstudio Cloud releases a new build hash. Auto-fetched on push retry (v1.2+) so manual call rarely needed. Do NOT use when: VERSION_MISMATCHED was already auto-resolved. Returns: new version + previous. Side effects: writes to local auth file. Example: {action:"update_app_version",label:"bump-version",projectSlug:"my-site",appVersion:"abc123def456..."}`,
};
