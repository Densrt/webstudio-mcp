...
const DESCRIPTIONS = {
  ...
  inspect: `Use when: inspect ONE instance, form, or resource in depth. Required params depend on target:
  - target:"instance" → (pageId | pagePath) + instanceIds → returns props + styles + immediate children summary.
  - target:"form" → (pageId | pagePath) → returns form fields, action, method, validation for every <Form> in scope.
  - target:"resource" → (resourceId | resourceName) → EXECUTES the HTTP resource and returns response shape + sample data.
Do NOT use when: scanning many items (use action:"list_instances"). Returns: structured details (one of the three shapes above). Side effects: none for instance/form, network read for resource. Example:
  ...
  snapshot: `Use when: capture PNG screenshot(s) of a specific instance (data-ws-id) via the Webstudio builder canvas (Playwright headless). Pass a single \`breakpoint\` for 1 capture, or \`breakpoints\` array (e.g. ["Base","Tablet","Mobile"]) for multi-breakpoint capture in one browser session (~5-7s total, faster than 3 separate calls). Do NOT use when: needing full-page only or visual diff without an instanceId (no full-page mode yet). Returns: content array with a metadata text + one image entry per captured breakpoint (type:"image", base64 PNG, mimeType:"image/png"). Side effects: 5-10s capture per call (Chromium launch + canvas hydration). Example single: {action:"snapshot",label:"check-hero",projectSlug:"my-site",instanceId:"abc",breakpoint:"Base"}. Example multi: {action:"snapshot",label:"check-responsive",projectSlug:"my-site",instanceId:"abc",breakpoints:["Base","Tablet","Mobile"]}`,
};
