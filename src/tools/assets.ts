...
const DESCRIPTIONS = {
  upload: `Use when: upload an IMAGE or FONT (webp/png/jpg/svg/avif/woff/woff2/ttf) to a project. Pass filePath, base64Content, or url. Do NOT use when: replacing an existing asset everywhere (use action:"replace" after upload). Returns: {assetId: sha256 hex string} — pass to instance_prop with type="asset" to bind on an Image's src, or to other actions that accept assetId. Side effects: push to Webstudio Cloud (uploads to CDN, requires allowPush). Example: {action:"upload",label:"upload-hero",projectSlug:"my-site",filePath:"/path/to/hero.webp"}`,
  ...
  find_usage: `Use when: locate every reference to a specific asset before deletion or replacement (props, styles, page meta). Do NOT use when: needing the full catalog (use action:"list"). Returns: asset header + per-kind references. Side effects: none (read-only). Example: {action:"find_usage",label:"audit-hero-usage",projectSlug:"my-site",assetName:"hero"}`,
  replace: `Use when: PROJECT-WIDE swap an asset everywhere (new logo with same dimensions, image variant update, file format upgrade). Do NOT use when: replacing in 1 spot only (use instances.prop_update). Returns: per-reference report (succeeded/failed). Side effects: push to Webstudio Cloud, CRITICAL — context required, affects ALL pages. Example: {action:"replace",label:"swap-logo-v2",projectSlug:"my-site",fromAssetId:"sha1...",toAssetId:"sha2...",context:"Updating the dealer logo across the entire site after the 2026 rebrand kickoff approved by the marketing lead last week",dryRun:true}`,
  delete: `Use when: BATCH delete assets (continue-on-error). Pass assetIdsOrPrefixes (each entry can be a full sha256 id or a prefix matching multiple assets). Do NOT use when: replacing an asset with a new version (use action:"replace" instead — preserves references). Returns: succeeded[]/failed[] report. Side effects: push to Webstudio Cloud, CRITICAL — context required, irreversible. Example: {action:"delete",label:"purge-orphan",projectSlug:"my-site",assetIdsOrPrefixes:["sha1","sha2"],context:"Removing 12 orphan images uploaded during the abandoned redesign experiment that are no longer referenced anywhere in the build",dryRun:true}`,
};
